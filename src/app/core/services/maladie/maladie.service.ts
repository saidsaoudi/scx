...
export class MaladieService {
  private readonly MALADIE = EndpointsConfig.api.maladie;

  constructor(private http: CustomHttp) { }

  searchMaladie(paginate: Boolean, paginator: Paginator, search: any){
    let queryParams = {skip: 0, take: 20, search:search, queryString: true}
    if(!queryParams.search || queryParams.search == ''){
      delete queryParams.search
    }
    return this.http.sendRequest(this.MALADIE.list, null, queryParams);
  }
}
